...
![v1-prod](https://img.shields.io/badge/version-v1--prod%20%E2%80%94%2091%2F91-brightgreen) ![tests](https://img.shields.io/badge/tests-497%20passed-brightgreen) ![license](https://img.shields.io/badge/license-MIT-blue)
...
The committed benchmark sweep tells the same story on a slightly different corpus. `benchmarks/results.md:13` records 22 files, 1450 nodes, 1450 clean C files, 16,620 C lines in 1303.0 ms. The corpus gate (`benchmarks/corpus`, 14 harder files — Kalman, control, ODE) produces 814 C files, 0 `#error`, all compiling with `-std=c99 -O2 -Wall -Wextra -pedantic` (see `benchmarks/baseline.json`). The hardened report pushes it to 91 kernels × 10k (92 total inc. unreachable `array_diff`) iterations adversarial without regressions.
...
* Stage 1 — Hardened Correctness: 91 kernels x 1k iter, sizes 16..4096, IEEE edge cases.
* Stage 2 — Brutal Scale: 91 kernels x 5k iter, sizes up to 16384.
* Stage 3 — Frontier Stress: 91 kernels x 10k iter, adversarial (VLA provocation, negative dims, zero), plus chaos corpora.
