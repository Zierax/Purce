...
def gradient_penalty(real_data, fake_data, discriminator_fn, lambda_gp=10.0):
    ...
    interpolated = np.add(np.multiply(alpha, real_data), np.multiply(np.subtract(1.0, alpha), fake_data))
...
def vae_kl_divergence(mu, logvar):
    return np.mean(np.add(np.multiply(-0.5, np.add(1.0, logvar)), np.multiply(-0.5, np.exp(logvar))))
...
def add_noise(x, alpha_bar_t):
    ...
    return np.add(np.multiply(np.sqrt(alpha_bar_t), x),
...
def spectral_norm(weight, n_power_iterations=1):
    ...
    for _ in range(n_power_iterations):
        v = np.divide(np.matmul(weight.T, u),
                      np.sqrt(np.add(np.sum(np.power(np.matmul(weight.T, u), 2)), 1e-12)))
        u = np.divide(np.matmul(weight, v),
                      np.sqrt(np.add(np.sum(np.power(np.matmul(weight, v), 2)), 1e-12)))
    sigma = np.sum(np.multiply(np.matmul(weight, u), v))
    return np.divide(weight, sigma)


def hinge_loss_d(real_scores, fake_scores):
    return np.add(np.mean(np.maximum(0, np.subtract(1.0, real_scores))),
