...
_BINOP = {
    ast.Add: "p_add", ast.Sub: "p_sub", ast.Mult: "p_mul",
    ast.Div: "p_truediv", ast.FloorDiv: "p_floordiv",
    ast.Mod: "p_mod", ast.Pow: "p_pow",
}

_CMPOP = {
    ast.Eq: "p_eq", ast.NotEq: "p_ne", ast.Lt: "p_lt",
    ast.LtE: "p_le", ast.Gt: "p_gt", ast.GtE: "p_ge",
}
...
class Emitter:
    ...
    def __init__(self, source: str) -> None:
        ...
        try:
            ...
            raise UnsupportedError(
                ast.Module(body=[]), f"syntax error in source: {e}"
            ) from None
        self.tmpn = 0
        self.fncount = 0
    ...
    def gen_function(self, fn: ast.FunctionDef, parent: _Frame) -> str:
        ...
        if a.vararg is not None or a.kwarg is not None or a.posonlyargs \
                or a.kwonlyargs or a.defaults or a.kw_defaults:
            raise UnsupportedError(
                fn, f"function '{fn.name}': defaults/varargs/kwonly/"
                    f"posonly args unsupported"
            )
        if not _IDENT.match(fn.name):
            raise UnsupportedError(fn, f"function name '{fn.name!r}' not "
                                       f"an ASCII identifier")
        ...
        for i, name in enumerate(argnames):
            lines.append(f"    p_frame_set(S, F, {frame.map[name]}, "
                         f"argv[{i}]);")
    ...
    def emit(em: Emitter, frame: _Frame, node) -> list[str]:
        ...
        if isinstance(node, ast.Assign):
            if len(node.targets) != 1:
                raise UnsupportedError(node, "multi-target assignment "
                                             "unsupported")
            t = node.targets[0]
        ...
        if isinstance(node, ast.AugAssign):
            ...
            if fn is None:
                raise UnsupportedError(node, f"unsupported augassign op "
                                              f"{type(node.op).__name__}")
            b = expr(node.value)
            if isinstance(node.target, ast.Name):
                if node.target.id not in frame.assigned:
                    raise UnsupportedError(node.target, f"augassign makes "
                                                        f"'{node.target.id}' "
                                                        f"a local but it is "
                                                        f"never assigned in "
                                                        f"this scope "
                ...
                raise UnsupportedError(node.target, "unsupported augassign "
                                                     "target")
            return lines
        ...
        if isinstance(node, ast.Assert):
            if node.msg is not None:
                raise UnsupportedError(node, "assert with message "
                                             "unsupported")
            test = expr(node.test)
            lines.append(
                f"    if (!p_truth(S, {test})) "
        ...
        if isinstance(node, ast.FunctionDef):
            ...
            v = em.tmp()
            lines.append(f"    pval *{v} = p_mkfunc(S, {cname}, F, "
                         f"{len(node.args.args)}, {c_str(node.name)});")
            lines.append(f"    if (!{v}) return NULL;")
            lines.append(f"    p_frame_set(S, F, {frame.alloc(node.name)}, "
                         f"{v});")
        ...
        if isinstance(node, (ast.Import, ast.ImportFrom, ast.ClassDef,
            ...
            raise UnsupportedError(node, f"{type(node).__name__} statements "
                                         f"unsupported")
        raise UnsupportedError(node, f"statement {type(node).__name__} "
                                     f"unsupported")
    ...
    def for_(em, frame, node, lines) -> list[str]:
        ...
        if not isinstance(target, ast.Name):
            if isinstance(target, (ast.Tuple, ast.List)):
                raise UnsupportedError(target, "tuple targets in for "
                                               "unsupported")
...
class _Expr:
    ...
    def emit(em: Emitter, frame: _Frame, node, lines) -> str:
        ...
        if isinstance(node, ast.Name):
            if (node.id in frame.assigned
                    and node.id not in frame.assigned_so_far):
                raise UnsupportedError(node, f"local variable '{node.id}' "
        ...
        if isinstance(node, (ast.Lambda, ast.ListComp, ast.SetComp,
                ...
                ast.YieldFrom, ast.Set, ast.Slice, ast.Starred,
                ast.Attribute)):
            raise UnsupportedError(node, f"{type(node).__name__} expression "
                                         f"unsupported")
        raise UnsupportedError(node, "unsupported expression node "
    ...
    def _const(em, node, lines) -> str:
        ...
        if isinstance(v, int):
            ...
            lines.append(f"    if (!p_int_from_str(S, {c_str(s)}, "
                         f"{len(s)}, &{t})) return NULL;")
            return t
    ...
    def _binop(em, frame, node, lines) -> str:
        fn = _BINOP.get(type(node.op))
        if fn is None:
            raise UnsupportedError(node, f"operator {type(node.op).__name__} "
                                         f"unsupported")
        ...
        def one(op, a, b, la, ra) -> str:
            ...
            if isinstance(op, (ast.Is, ast.IsNot)):
                if not (is_none_lit(la) or is_none_lit(ra)):
                    raise UnsupportedError(node, "'is'/'is not' supported "
                                                 "only with a None literal")
                opc = "==" if isinstance(op, ast.Is) else "!="
                lines.append(f"    pval *{t} = p_newbool(S, ({a}) {opc} "
                             f"({b}));")
                return t
            if isinstance(op, ast.In):
                lines.append(f"    pval *{t} = p_newbool(S, p_contains(S, "
                             f"{b}, {a}));")
                ...
                if fn is None:
                    raise UnsupportedError(node, f"comparison "
                                                 f"{type(op).__name__} "
                                                 f"unsupported")
                lines.append(f"    pval *{t} = p_newbool(S, {fn}(S, {a}, "
                             f"{b}));")
            lines.append("    if (p_err(S)) return NULL;")
            return t
    ...
    def _call(em, frame, node, lines) -> str:
        ...
        if r is not None:
            ...
            lines.append(f"    pval *{t} = p_call(S, {fv}, {len(args)}, "
                         f"{arr});")
        elif name in BUILTINS:
            lines.append(f"    pval *{t} = p_builtin_call(S, {c_str(name)}, "
                         f"{len(args)}, {arr});")
        else:
            raise UnsupportedError(node, f"function or builtin '{name}' "
                                         f"not defined")
        lines.append(f"    if (!{t}) return NULL;")
        return t
